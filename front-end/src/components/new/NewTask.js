...
import { newTaskFunc } from '../../redux/stateSlice/taskState';
...
const NewTask = () => 
{
            ...
            if (result.isConfirmed) {
              ...
              (
                (res)=>
                {
                    if(res===true)
                    {
                        findTaskByStatus("new").then //after delete it will refresh page 
                        (
                            (res)=>
                            {
                                if(res!==false)
                                {
                                    dispatch(newTaskFunc(res))
                                }
                                else
                                {
    ...
    var newTask = useSelector((state)=>state.taskState.new)
    var newTaskArr = newTask.map(
        function(p1)
        {
            return(
                <div className='col'>
                        ...
                        <div className='complete-task-card-inner-grid'>
                            ...
                            <div>
                                <button className='new-task-status-btn'>{p1.status}</button>
                            </div>
                        </div>
                </div>
            )
