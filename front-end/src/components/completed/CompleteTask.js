...
import { completedTaskFunc } from '../../redux/stateSlice/taskState';
...
const CompleteTask = () => 
{
            ...
            if (result.isConfirmed) {
              ...
              (
                (res)=>
                {
                    if(res===true)
                    {
                        findTaskByStatus("complete").then  //after delete it will refresh page 
                        (
                            (res)=>
                            {
                                if(res!==false)
                                {
                                    dispatch(completedTaskFunc(res))
                                }
                                else
                                {
    ...
    useEffect(()=>{
        ...
        findTaskByStatus("complete").then
        (
            (res)=>
            {
                if(res!==false)
    ...
    var completeTask = useSelector((state)=>state.taskState.completed)
    var completeTaskArr = completeTask.map(
        function(p1)
        {
            return(
                <div className='col'>
                        ...
                        <div className='complete-task-card-inner-grid'>
                            ...
                            <div>
                                <button className='complete-task-status-btn'>{p1.status}</button>
                            </div>
                        </div>
                </div>
            )
