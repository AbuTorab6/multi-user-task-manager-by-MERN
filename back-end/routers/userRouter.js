...
var registration = async (req,res)=>
{
    ...
    if(serach[0]==undefined)
    {
        ...
        try
        {
            ...
            {
                res.status(400);
                res.send("data can not inserted for registration!.");
            }
            else
            {
                res.status(200);
                res.send("user is registared.")
            }
        }
        catch(ob)
        {
...
var login = async(req,res)=>
{
    ...
    else
    {
        ...
        {
            var token = jwt.sign({firstName:serach[0].firstName,lastName:serach[0].lastName,email:dataFromPostman.email},process.env.KEY);


            var serach = await userModel.aggregate([
                {$match:{email:{$eq:dataFromPostman.email}}},
                {$project:{firstName:1,lastName:1,email:1,photo:1,mobile:1,_id:0}}
            ])
            res.status(200);
            res.json({token:token,staus:"success",data:serach[0]})
        }
        else
        {
            res.status(203);
    ...
    try
        {
            ...
            else
            {
                ...
                let email = await sendEmail(myEmail,"Your PIN Code is= "+OTPCode,"Task Manager PIN Verification")
